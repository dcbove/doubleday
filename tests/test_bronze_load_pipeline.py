"""Unit tests for doubleday.lambdas.bronze_load.pipeline."""
...
from doubleday.lambdas.bronze_load.pipeline import (
    LoadResult,
    _object_exists,
    _s3_key,
    download_partition,
...
class TestDownloadPartition:
    """Tests for download_partition."""

    @patch("doubleday.lambdas.bronze_load.pipeline._object_exists", return_value=True)
    ...
    @patch("doubleday.lambdas.bronze_load.pipeline.urlopen")
    @patch("doubleday.lambdas.bronze_load.pipeline._object_exists", return_value=True)
    ...
    @patch("doubleday.lambdas.bronze_load.pipeline.urlopen")
    @patch("doubleday.lambdas.bronze_load.pipeline._object_exists", return_value=False)
