"""Unit tests for doubleday.lambdas.clear_staging.handler."""
...
from doubleday.lambdas.clear_staging.handler import handler  # noqa: E402


class TestClearStagingHandler:
    """Tests for the clear_staging handler."""

    @patch("doubleday.lambdas.clear_staging.handler.SQL_DIR")
    @patch("doubleday.lambdas.clear_staging.handler.run_query")
    @patch("doubleday.lambdas.clear_staging.handler.metrics")
    @patch("doubleday.lambdas.clear_staging.handler.athena")
    @patch("doubleday.lambdas.clear_staging.handler.DATABASE", "test_db")
    @patch("doubleday.lambdas.clear_staging.handler.OUTPUT_BUCKET", "test-bucket")
