"""Unit tests for the query_neighbors handler (doubleday.api.query_neighbors.handler).
...
from doubleday.api.query_neighbors.handler import handler  # noqa: E402
from doubleday.api.query_neighbors.query import QueryResult  # noqa: E402
...
def lambda_context():
    ...
    ctx = MagicMock()
    ctx.function_name = "test-query-neighbors"
...
def _event(principal_id="user-123", pitcher_id="12345", season="2025"):
    """Build an API Gateway proxy event for GET /pitchers/{id}/neighbors."""
    return {
...
class TestRequireSubscriptionEnabled:
    ...
    @patch("doubleday.api.query_neighbors.handler.REQUIRE_SUBSCRIPTION", True)
    @patch("doubleday.api.query_neighbors.handler.dynamodb")
    ...
    @patch("doubleday.api.query_neighbors.handler.REQUIRE_SUBSCRIPTION", True)
    @patch("doubleday.api.query_neighbors.handler.query_neighbors")
    @patch("doubleday.api.query_neighbors.handler.dynamodb")
...
class TestRequireSubscriptionDisabled:
    ...
    @patch("doubleday.api.query_neighbors.handler.REQUIRE_SUBSCRIPTION", False)
    @patch("doubleday.api.query_neighbors.handler.query_neighbors")
    @patch("doubleday.api.query_neighbors.handler.dynamodb")
...
class TestDefaultRequireSubscription:
    ...
    def test_default_requires_subscription(self):
        ...
        try:
            ...
            import doubleday.api.query_neighbors.handler as mod
