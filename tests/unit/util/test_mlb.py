...
This module provides functions for fetching player enrichment data and team
metadata from the MLB Stats API. It batches player requests (200 per call),
handles partial failures gracefully, and normalizes player names for search.

These tests mock urllib.urlopen to verify: team parsing, player enrichment
extraction, batching logic, partial failure handling, empty input, and
name normalization (diacritics, punctuation, casing).
"""
